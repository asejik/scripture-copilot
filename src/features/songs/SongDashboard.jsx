...
import ScaledPreview from '../../components/ScaledPreview'; // NEW
...
const SongDashboard = () => {
  ...
  const LibraryPanel = (
    <div className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden shadow-2xl flex flex-col h-full">
        ...
        <div className="p-3 bg-slate-950 border-b border-slate-800 shrink-0">
            <input type="text" placeholder="Search songs..." className="w-full bg-slate-900 border border-slate-700 rounded px-3 py-2 text-xs text-white focus:outline-none focus:border-blue-500 transition-colors" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {filteredSongs.length === 0 ? (
                ...
                filteredSongs.map(song => (
                    <div key={song.id} onClick={() => setActiveSong(song)} className={`p-3 rounded border cursor-pointer group transition-colors flex justify-between items-center ${activeSong?.id === song.id ? 'bg-blue-900/20 border-blue-500/50' : 'bg-transparent border-transparent hover:bg-slate-800 hover:border-slate-700'}`}>
                        ...
                        <button onClick={(e) => { e.stopPropagation(); addToAgenda(song); }} className="opacity-0 group-hover:opacity-100 bg-slate-700 hover:bg-green-600 text-white text-[10px] px-2 py-1 rounded transition-all" title="Add to Agenda">+ Add</button>
                    </div>
                ))
            )}
        </div>
                ...
                <div className="flex-1 p-4 bg-slate-950 overflow-y-auto space-y-4">
                    {getSlides(activeSong.lyrics).map((slide, idx) => (
                        <div key={idx} className="group relative pl-6">
                            ...
                            <div onClick={() => handleProjectSlide(idx)} className="text-slate-300 text-sm leading-relaxed p-4 rounded bg-slate-900/50 border border-slate-800 hover:border-purple-500/50 hover:bg-slate-900 cursor-pointer transition-all whitespace-pre-wrap relative">
                                {slide}
                                <button className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 bg-purple-600 hover:bg-purple-500 text-white text-[10px] px-3 py-1 rounded shadow-lg font-bold transition-all">PROJECT</button>
                            </div>
                        </div>
                    ))}
                </div>
  ...
  const AgendaPanel = (
    <div className="flex flex-col h-full bg-slate-900">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {agenda.length === 0 ? (
                <div className="text-center text-slate-600 mt-10 text-xs italic">Agenda empty.<br/>Add songs from Library.</div>
            ) : (
                agenda.map((item, idx) => (
                    <div key={item.agendaId} onClick={() => setActiveSong(item)} className={`flex items-center gap-2 p-2 rounded border cursor-pointer transition-colors group ${activeSong?.id === item.id ? 'bg-slate-800 border-slate-600' : 'bg-slate-800/30 border-slate-700 hover:bg-slate-800'}`}>
                        ...
                        <button onClick={(e) => { e.stopPropagation(); removeFromAgenda(item.agendaId); }} className="text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity px-2">×</button>
                    </div>
                ))
            )}
        </div>
        ...
        <div className="flex-1 flex items-center justify-center p-4 bg-black overflow-hidden relative">
            {liveScripture ? (
                <ScaledPreview aspectRatio={aspectRatio}>
                    <SlideRenderer content={liveScripture} theme={theme} fontSize={fontSize} layoutMode={layoutMode} textAlign={textAlign} aspectRatio={aspectRatio} fontFamily={fontFamily} textTransform={textTransform} isPreview={false} />
                </ScaledPreview>
            ) : (
  ...
  return (
    <div className="w-full h-[calc(100vh-6rem)] relative">
      <SongEditorModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSave} songToEdit={editingSong} />
      <ResizableGrid
        left={LibraryPanel}
